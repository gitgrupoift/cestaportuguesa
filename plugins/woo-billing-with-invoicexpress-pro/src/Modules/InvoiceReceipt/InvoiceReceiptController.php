<?php
namespace Webdados\InvoiceXpressWooCommerce\Modules\InvoiceReceipt;
...
class InvoiceReceiptController extends BaseController {
	...
	public function register_hooks() {
		...
		if ( get_option( 'hd_wc_ie_plus_invoice_receipt' ) ) {
			add_filter(
			...
			add_action(
				'woocommerce_order_action_hd_wc_ie_plus_generate_invoice_receipt', array(
					$this,
					'doAction',
				), 10, 2
			);
	...
	public function order_actions( $actions ) {
		...
		if ( $has_scheduled ) {
			if ( apply_filters( 'invoicexpress_woocommerce_check_pending_scheduled_document', false, $order_object->get_id(), array( 'invoice_receipt' ) ) ) {
				//Has Invoice-receipt scheduled - Clock
				$symbol = '&#x1f550;';
			} else {
			...
			if ( empty( $invoice_id ) && empty( $simplified_invoice_id ) && empty( $invoice_receipt_id ) && empty( $vat_moss_invoice_id ) ) {
				...
				$symbol = '&#xd7;';
				if ( ! empty( $invoice_receipt_id ) ) {
					//There's already a Invoice-receipt - Check
					$symbol = '&#x2713;';
				}
			}
		}

		$actions['hd_wc_ie_plus_generate_invoice_receipt'] = trim( sprintf(
			'%s %s: %s',
			$symbol,
	...
	public function doAction( $order_object, $mode = 'manual' ) {
		...
		$debug = 'Checking if Invoice-receipt document should be issued';
		...
		if (
			...
			$items_data = $this->getOrderItemsForDocument( $order_object, 'invoice_receipt' );
			...
			$invoice_data = array(
				...
				'client'           => $client_data,
				'items'            => $items_data,
				'sequence_id'      => $this->find_sequence_id( $order_object->get_id(), 'invoice_receipt' ),
			...
			$invoice_data = $this->process_items( $invoice_data, $order_object, 'invoice_receipt' );

			$invoice_data = apply_filters( 'invoicexpress_woocommerce_invoice_receipt_data', $invoice_data, $order_object );

			//Prevent issuing?
			$prevent = $this->preventDocumentIssuing( $order_object, 'invoice_receipt', $invoice_data, $mode );
			if ( isset( $prevent['prevent'] ) && $prevent['prevent'] ) {
				$this->preventDocumentIssuingLogger( $prevent, 'invoice_receipt', $order_object, $mode );
				return;
			}
			...
			if ( ! $return['success'] ) {
				...
				do_action( 'invoicexpress_woocommerce_error', 'Issue Invoice-receipt: '.$error_notice, $order_object );
				return;
			}
			
			$order_id_invoicexpress = $return['object']->invoice_receipt->id;
			...
			//Update invoice data
			$order_object->update_meta_data( 'hd_wc_ie_plus_invoice_receipt_id', $order_id_invoicexpress );
			$order_object->update_meta_data( 'hd_wc_ie_plus_invoice_receipt_permalink', $return['object']->invoice_receipt->permalink );
			$order_object->update_meta_data( 'hd_wc_ie_plus_invoice_type', 'invoice_receipt' );
			$order_object->save();

			do_action( 'invoicexpress_woocommerce_after_document_issue', $order_object->get_id(), 'invoice_receipt' );
			...
			if ( get_option( 'hd_wc_ie_plus_leave_as_draft' ) ) {
				...
				/* Leave as Draft */
				$this->draft_document_note( $order_object, __( 'Invoice-receipt', 'woo-billing-with-invoicexpress' ) );
				...
				/* Change document state to final and store sequence number */
				$return = $this->changeOrderState( $order_id_invoicexpress, 'finalized', 'invoice_receipt' );
				if ( ! $return['success'] ) {
					$codeStr    = __( 'Code', 'woo-billing-with-invoicexpress' );
					$messageStr = __( 'Message', 'woo-billing-with-invoicexpress' );
					/* Add notice */
					...
					do_action( 'invoicexpress_woocommerce_error', 'Change Invoice-receipt state to finalized: '.$error_notice, $order_object );
					return;
				} else {
					$notice = sprintf(
						'<strong>%s:</strong> %s',
				...
				$sequence_number = $return['object']->invoice_receipt->inverted_sequence_number;
				$order_object->update_meta_data( 'hd_wc_ie_plus_invoice_receipt_sequence_number', $sequence_number );
				$order_object->save();
				...
				do_action( 'invoicexpress_woocommerce_before_document_email', $order_object->get_id(), 'invoice_receipt' );

				/* Get and send the PDF */
				if ( ! $this->getAndSendPDF( $order_object, 'invoice_receipt', $order_id_invoicexpress, $mode ) ) {
					return;
				}

				do_action( 'invoicexpress_woocommerce_after_document_finish', $order_object->get_id(), 'invoice_receipt' );
			}
