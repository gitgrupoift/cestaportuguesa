<?php
namespace Webdados\InvoiceXpressWooCommerce\Modules\Quote;
...
class QuoteController extends BaseController {
	...
	public function register_hooks() {
		...
		if ( get_option( 'hd_wc_ie_plus_quote' ) ) {
			add_filter(
			...
			add_action(
				'woocommerce_order_action_hd_wc_ie_plus_generate_quote', array(
					$this,
					'doAction',
				), 10, 1
			);
	...
	public function order_actions( $actions ) {
		...
		$quote_id = $order_object->get_meta( 'hd_wc_ie_plus_quote_id' );
		if ( empty( $quote_id ) ) {
			$symbol = '';
		} else {
			$symbol = '&#x2713;';
		...
		$actions['hd_wc_ie_plus_generate_quote'] = trim( sprintf(
			'%s %s: %s',
			$symbol,
	...
	public function doAction( $order_object, $mode = 'manual' ) {
		...
		$debug = 'Checking if Quote document should be issued';
		do_action( 'invoicexpress_woocommerce_debug', $debug, $order_object, array(
			'hd_wc_ie_plus_quote_id'  => $quote_id,
		) );
		...
		if ( empty( $quote_id ) ) {
			...
			if ( get_option( 'hd_wc_ie_plus_vat_field_mandatory' ) && empty( $vat ) ) {
				...
				if ( $mode == 'manual' ) { //Always manual in Quotes, but we may change that in the future
					Notices::add_notice(
						$error_notice,
						'error'
					);
			...
			$items_data = $this->getOrderItemsForDocument( $order_object, 'quote' );
			...
			$invoice_data = array(
				...
				'client'           => $client_data,
				'items'            => $items_data,
				'sequence_id'      => $this->find_sequence_id( $order_object->get_id(), 'quote' ),
				//'owner_invoice_id' => '', //We shouldn't need to associate Quotes with any other document
			...
			$invoice_data = $this->process_items( $invoice_data, $order_object, 'quote' );

			$invoice_data = apply_filters( 'invoicexpress_woocommerce_quote_data', $invoice_data, $order_object );

			//Prevent issuing?
			$prevent = $this->preventDocumentIssuing( $order_object, 'quote', $invoice_data, $mode );
			if ( isset( $prevent['prevent'] ) && $prevent['prevent'] ) {
				$this->preventDocumentIssuingLogger( $prevent, 'quote', $order_object, $mode );
				return;
			}
			...
			if ( ! $return['success'] ) {
				...
				if ( get_option( 'hd_wc_ie_plus_automatic_email_errors' ) && $mode == 'automatic' && $error_notice ) { //Always manual in Quotes, but we may change that in the future
					$this->sendErrorEmail( $order_object, $error_notice );
				}
				do_action( 'invoicexpress_woocommerce_error', 'Issue Quote: '.$error_notice, $order_object );
				return;
			}

			$order_id_invoicexpress = $return['object']->quote->id;
			...
			//Update quote data
			$order_object->update_meta_data( 'hd_wc_ie_plus_quote_id', $order_id_invoicexpress );
			$order_object->update_meta_data( 'hd_wc_ie_plus_quote_permalink', $return['object']->quote->permalink );
			$order_object->save();

			do_action( 'invoicexpress_woocommerce_after_document_issue', $order_object->get_id(), 'quote' );
			...
			if ( get_option( 'hd_wc_ie_plus_leave_as_draft' ) ) {
				...
				/* Leave as Draft */
				$this->draft_document_note( $order_object, __( 'Quote', 'woo-billing-with-invoicexpress' ) );
				...
				$return = $this->changeOrderState( $order_id_invoicexpress, 'finalized', 'quote' );
				if ( ! $return['success'] ) {
					$codeStr    = __( 'Code', 'woo-billing-with-invoicexpress' );
					$messageStr = __( 'Message', 'woo-billing-with-invoicexpress' );
					/* Add notice */
					...
					do_action( 'invoicexpress_woocommerce_error', 'Change Quote state to finalized: '.$error_notice, $order_object );
					return;
				} else {
					$notice = sprintf(
						'<strong>%s:</strong> %s',
				...
				$sequence_number = $return['object']->quote->inverted_sequence_number;
				$order_object->update_meta_data( 'hd_wc_ie_plus_quote_sequence_number', $sequence_number );
				$order_object->save();
				...
				do_action( 'invoicexpress_woocommerce_before_document_email', $order_object->get_id(), 'quote' );

				/* Get and send the PDF */
				if ( ! $this->getAndSendPDF( $order_object, 'quote', $order_id_invoicexpress, $mode ) ) {
					return;
				}

				do_action( 'invoicexpress_woocommerce_after_document_finish', $order_object->get_id(), 'quote' );
			}
