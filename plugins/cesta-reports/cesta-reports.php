<?php
/**
 * Plugin Name: Cesta Portuguesa Relatórios
 * Description: Customização de relatórios para a equipa do Cesta Portuguesa.
 * Version: 1.0.0
 * Author: Carlos Matos
 * text-domain: cesta-reports
...
function hm_psrf_admin_menu() {
	add_submenu_page('woocommerce', 'Cesta Relatórios', 'Cesta Relatórios', 'view_woocommerce_reports', 'hm_sbpf', 'hm_sbpf_page');
}
...
function hm_sbpf_page() {
	...
	$fieldOptions = array(
		...
		'variation_attributes' => 'Atributos Variáveis',
		'quantity_sold' => 'Quantidade',
		'gross_sales' => 'Vendas Brutas',
		'gross_after_discount' => 'Venda Após Descontos'
	);
	...
	foreach ($_POST['fields'] as $field) {
		switch ($field) {
			...
			case 'variation_attributes':
				$header[] = 'Atributos Variáveis';
				break;
			case 'quantity_sold':
				$header[] = 'Quantidade';
				break;
			case 'gross_sales':
				$header[] = 'Vendas Brutas';
				break;
			case 'gross_after_discount':
				$header[] = 'Venda Após Descontos';
				break;
