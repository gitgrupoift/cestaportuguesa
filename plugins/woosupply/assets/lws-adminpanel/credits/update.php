...
class Update
{
	...
	public function check_update($transient)
	{
		if( !empty($transient) )
		{
			...
			{
				if( isset($remote_info->new_version) )
				{
			...
			else if( !empty(\apply_filters('lws-ap-release-' . $this->slug, '')) )
			{
				// remove wp info since we only trust lws for pro
				if( isset($transient->response[$this->plugin_slug]) )
					unset($transient->response[$this->plugin_slug]);
				$this->noticeAboutSilentServer();
			}
		}
		return $transient;
	}
	...
	private function noticeAboutSilentServer()
	{
		if( isset($this->lastInfoError) && $this->lastInfoError && !empty(\apply_filters('lws-ap-release-' . $this->slug, '')) )
		{
			$kwarn = 'lws_ap_last_silent_server';
			$lastWarn = \get_site_option($kwarn, '1.0.0');
			$currentWarn = $this->pluginData('Version');
			if( !$lastWarn || !$currentWarn || \version_compare($lastWarn, $currentWarn, '<') )
			{
				/// warn user about silent server.
				/// Known issu with some OVH clusters rejecting out to some ip with mysterious rules
				\lws_admin_add_notice($kwarn,
					$this->silentServerMessage(),
					array('level'=>'warning', 'dismissible'=>'on', 'forgettable'=>'on')
				);
				\update_site_option($kwarn, $currentWarn ? $currentWarn : '1.0.0');
			}
		}
	}

	private function silentServerMessage()
	{
		$url = array_merge(array('scheme'=>'https', 'hostname'=>'plugins.longwatchstudio.com'), \parse_url($this->remote_path));
		$msg = sprintf(
			__("<p><b>%s</b> cannot contact licence server. Plugin update encounters difficulties.</p>", LWS_ADMIN_PANEL_DOMAIN),
	...
	private function getRemoteInfo()
	{
		if( !isset($this->info) )
		{
			...
			$this->lastInfoError = false;
			if( \is_wp_error($request) )
			{
				$this->lastInfoError = true;
	...
	{
		if( !$this->isAboutMe($package, $upgrader) ) return $reply;
		if( false !== $reply ) return $reply;
		if( false === ($info = $this->getRemoteInfo()) )
		{
			if( !empty(\apply_filters('lws-ap-release-' . $this->slug, '')) )
				return new \WP_Error('silent_server', $this->silentServerMessage());
			return $reply;
		}
	...
	function isAboutMe($package, $upgrader)
	{
		if( !$upgrader ) return false; // cannot guess
		if( !isset($upgrader->skin->plugin_info) ) return false; // cannot guess
		if( !isset($upgrader->skin->plugin_info['Name']) ) return false; // cannot guess
		return $upgrader->skin->plugin_info['Name'] === $this->pluginData('Name');
	}
