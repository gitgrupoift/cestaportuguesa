...
class Query
{
	...
	public static function install($main_file, $api_url='', $adminPageId='', $nic='')
	{
		...
		if( !isset($slugDone[$me->slug]) )
		{
			...
			if( defined('DOING_AJAX') )
			{
				...
				add_action('wp_ajax_lws_adminpanel_alt_activation', array($me, 'grabSecondChance'));
			}
			else
			{
				//if( !$activated )
	...
	private function addFormScript()
	{
		...
		wp_enqueue_script( $guid, $script, array('jquery', 'lws-base64'), LWS_ADMIN_PANEL_VERSION, true );
...
//		$url = \wp_nonce_url($url, 'upgrade-plugin_' . $this->plugin_slug);
	...
	/** @return another form for alternative activation. */
	private function secondChance()
	{
		error_log("Second chance: alternative activation form.");

		$url = array_merge(array('scheme'=>'https', 'hostname'=>'plugins.longwatchstudio.com'), \parse_url($this->url));
		$msg = __("<p>Your web host cannot contact the licence server. Update and activation could be affected.</p>", LWS_ADMIN_PANEL_DOMAIN);
		$msg .= sprintf(
			__("<p>It is a known issue on some web host providers working with clusters. Please contact your host provider and ask for <i>%s</i></p>", LWS_ADMIN_PANEL_DOMAIN),
			"{$url['scheme']}://{$url['hostname']}"
		);
		$msg .= __("<p>Please fill in the form again to try activation in <b>degraded mode</b></p>", LWS_ADMIN_PANEL_DOMAIN);
		...
		$ph = array(
			...
			base64_encode(json_encode(array(
				'fingerprint' => $fp,
				'version' => $data['Version'],
				'hash' => hash('sha256',implode('/', array($this->prod, $data['Version'], $fp, $nonce, $this->nic))),
				'nonce' => $nonce,
				'alt' => base64_encode($nonce),
			))),
			esc_attr(admin_url('admin-ajax.php?action=lws_adminpanel_alt_activation')),
		);

		$msg .= <<<EOT
<p class='lws-alt-activation'>
	<label>
		<span class='lws-adminpanel-activation-form-title'>{$ph[1]}</span>
		<span class='lws-adminpanel-activation-form-input'>
			<input class='lws-input lws-ignore-confirm' type='text' name='license_token' size='25' required />
		</span>
	</label>
	<input class='lws-adm-btn' type='submit' value='{$ph[0]}' data-nonce='{$ph[2]}' data-service='{$ph[3]}' data-info='{$ph[4]}' data-alt='{$ph[5]}'>
</p>
EOT;
		return $msg;
	}

	function grabSecondChance()
	{
		$fingerprint = $this->fingerprint();
		if( isset($_POST['fingerprint']) && $_POST['fingerprint'] === $fingerprint )
		{
			$result['ok'] = false;
			$result['html'] = __("<p>Internal error, please retry later.</p>", LWS_ADMIN_PANEL_DOMAIN);

			$expect = array('fingerprint', 'slug', 'ok', 'hash', 'token', 'nonce', 'rand');
			$vals = array_intersect_key($_POST, array_combine($expect, $expect));
			if( isset($_COOKIE['nonce']) ) $vals['nonce'] = $_COOKIE['nonce'];
			if( isset($_COOKIE['rand']) ) $vals['rand'] = $_COOKIE['rand'];

			if( count($vals) == count($expect) && $vals['rand'] && $vals['nonce'] && wp_verify_nonce($vals['nonce'], $fingerprint . $this->slug) && ($vals['fingerprint'] === $fingerprint) )
			{
				if( isset($_POST['html']) && !empty($_POST['html']) )
					$vals['html'] = \sanitize_text_field($_POST['html']);
				if( isset($_POST['expire']) && !empty($_POST['expire']) )
					$vals['expire'] = \sanitize_text_field($_POST['expire']);
				$result = $this->grab($result, $fingerprint, (object)$vals, $vals['nonce'], $vals['rand']);
			}
			else
				$result['html'] = __("<p>Corrupted data.</p>", LWS_ADMIN_PANEL_DOMAIN);
			wp_send_json($result);
		}
	}
	...
	private function askLicenseServer($args, $result, $fingerprint)
	{
		...
		else
		{
			...
			$result = $this->grab($result, $fingerprint, $json, $nonce, $rand);
		}
		return $result;
	}

	private function grab($result, $fingerprint, $json, $nonce, $rand)
	{
