...
abstract class Wizard
{
	...
	{
		return sprintf("<img style='width:300px;' src='%s'>", \esc_attr($this->getLogoURL()));
	}
	...
	protected function getHead()
	{
		...
		$title = $this->getTitle();
		return <<<EOT
<div class='lws-wizard-logo-container'>
	{$img}
</div>
<div class='lws-wizard-title-container'>
	{$title}
</div>
EOT;
	}
	...
	protected function getPathTabs($criticalPath)
	{
		...
		foreach($criticalPath as $step => $item)
		{
			...
			if( isset($item['label']) && $item['label'] )
			{
				...
				$tabs[$step] = <<<EOT
<{$tag} class='lws-wizard-progress-item {$state}'{$attr}>
	<div class='lws-wizard-progress-item-line'></div>
	<div class='lws-wizard-progress-item-text'>{$item['label']}</div>
</{$tag}>
EOT;
			}
		}
	...
	function wizard()
	{
		...
		echo <<<EOT
<div class="lws_wizard">
	<form $formAttrs>
		<input type='hidden' name='step' value='{$eStep}'>
		<input type='hidden' name='loop' value='{$loop}'>
		<input type='hidden' name='timestamp' value='{$time}'>
		{$nonce}
		<div class="lws-wizard-big-container">
			{$head}
			<div class='lws-wizard-progress-container'>
				{$tabs}
			</div>
EOT;

		if( $title = isset($page['title']) ? trim($page['title']) : '' )
			$title = "<div class='lws-wizard-form-title'>{$title}</div>";
		if( $help = isset($page['help']) ? trim($page['help']) : '' )
			$help = "<div class='lws-wizard-form-help'>{$help}</div>";
		if( isset($this->lastError) && $this->lastError )
		{
			$err = implode('</li><li>', $this->lastError);
			$help .= "<div class='lws-wizard-form-help lws-wizard-form-error'><ul><li>{$err}</li></ul></div>";
		}

		echo <<<EOT
			<div class='lws-wizard-main-container' data-step='{$eStep}' data-occurrence='$loop'>
				<div class='lws-wizard-form-title-line'>
					{$title}{$help}
				</div>
EOT;
		...
		{
			require_once LWS_ADMIN_PANEL_INCLUDES.'/pages/class-field.php';
		...
		$foot = $this->getFoot();
		echo <<<EOT
				<div class='lws-wizard-action-line'>
					{$buttons}
				</div>
			</div>
			{$foot}
...
EOT;
	...
	protected function groups(&$groups, $depth=0)
	{
		foreach( $groups as $group )
		{
			$style='';
			$vStyle='';
			$class = 'lws-wizard-form-grid group-depth-'.$depth;
			...
			if( isset($group['require']) )
			{
				...
				$attributes = " data-selector='{$s}' data-value='{$v}'";
			}
			...
			{
				echo "<div class='lws-wizard-group-title'>{$title}</div>";
			}
					...
					if( $field )
					{
						...
						if( !$field->isHidden() )
						{
							if( !empty($help = $field->help()) )
								echo "<div class='lws-wizard-form-item-help-grid'><div class='lws-wizard-form-item-help-lines'></div></div><div class='lws-wizard-form-item-help'>{$help}</div>";

							$title = $field->title();
							if( !empty($tooltips = $field->getTooltips()) )
								$tooltips = \lws_get_tooltips_html($tooltips);
							echo "<div class='lws-wizard-form-item-label'>{$title}{$tooltips}</div><div class='lws-wizard-form-item-value' style='{$vStyle}'>";
							$field->input();
							echo "</div>";
						}
						else
	...
	<button class='lws-wizard-action-back' name='submit' type='submit' value='previous'>
		<div class='lws-wizard-action-icon lws-icon-arrow-left'></div>
		<div class='lws-wizard-action-label'>{$button}</div>
	</button>
EOT;
		}

		$buttons['filler'] = "<div class='lws-wizard-action-filler'></div>";
		...
		if( isset($page['repeatable']) && $page['repeatable'] )
		{
			...
			$buttons['repeat'] = <<<EOT
	<button class='lws-wizard-action-redo' name='submit' type='submit' value='repeat'>
		<div class='lws-wizard-action-icon lws-icon-spinner11'></div>
		<div class='lws-wizard-action-label'>{$button}</div>
	</button>
EOT;
		}
		...
		$buttons['next'] = <<<EOT
	<button class='lws-wizard-action-next' name='submit' type='submit' value='{$value}'>
		<div class='lws-wizard-action-label'>{$button}</div>
		<div class='lws-wizard-action-icon lws-icon-arrow-right'></div>
	</button>
EOT;
		return $buttons;
	}
	...
	{
		$href = $this->getCancelledURL();
		$leave = sprintf('<span class="label">%s</span> %s', __("Leave this wizard", LWS_ADMIN_PANEL_DOMAIN), \lws_get_tooltips_html(__("You can come back later and continue from that point.")));
		$tooltips = \lws_get_tooltips_html(__("You will lose all prepared settings."));
		$label = __("Cancel this wizard", LWS_ADMIN_PANEL_DOMAIN);
		return <<<EOT
<div class='lws-wizard-cancel-container'>
	<button class='lws-wizard-action-cancel' name='submit' type='submit' value='cancel'>
		<div class='lws-wizard-action-cancel-icon lws-icon-close'></div>
		<div class='lws-wizard-action-cancel-text'>{$label}</div>
		{$tooltips}
	</button>
	<a href='{$href}'>
		{$leave}
		<div class='lws-wizard-action-cancel-icon lws-icon-sign-out'></div>
	</a>
</div>
EOT;
	}
